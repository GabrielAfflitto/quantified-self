...
const createInputField = (event) => {
  ...
  let type = $(`#${event.target.id}`).attr("class")
  $(`#${event.target.id}`).html(`<input class="${type}" type="text" placeholder=${event.target.innerHTML}  value="${event.target.innerHTML}"aria-required="true">`);
}
